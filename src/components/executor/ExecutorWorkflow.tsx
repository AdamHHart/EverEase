...
export default function ExecutorWorkflow() {
  ...
  const checkVerificationStatus = async () => {
    try {
      ...
      // Check if there's an onboarding session with completed steps
      const { data: sessionData } = await supabase
        .from('executor_onboarding_sessions')
        .select('current_step, completed_steps')
        .eq('executor_id', executorData.id)
        .eq('planner_id', executorData.planner_id)
        .single();

      if (sessionData && sessionData.completed_steps) {
        // Update steps based on completed steps in the session
        const completedSteps = sessionData.completed_steps;
        
        if (completedSteps.verification) {
          setIsVerified(true);
          steps[0].completed = true;
        }
        
        if (completedSteps.will) {
        ...
        if (completedSteps.contacts) {
          steps[4].completed = true;
        }
        
        // Set current step to the next uncompleted step
        if (sessionData.current_step > 0 && sessionData.current_step <= steps.length) {
          setCurrentStep(sessionData.current_step - 1);
        }
      }
  ...
  const markStepCompleted = async (stepIndex: number) => {
    ...
    if (executorData) {
      // Update the onboarding session if it exists
      const { data: sessionData } = await supabase
        .from('executor_onboarding_sessions')
      ...
      if (sessionData) {
        ...
        await supabase
          .from('executor_onboarding_sessions')
          .update({
            current_step: stepIndex + 2, // Next step (1-indexed)
