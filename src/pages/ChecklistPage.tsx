...
export default function ChecklistPage() {
  ...
  // Calculate completion percentage based on both checklist and actual progress
  const hasAssets = stats.assets > 0;
  const hasDocuments = stats.documents > 0;
  const hasWishes = stats.wishes > 0;
  const hasExecutors = stats.executors > 0;
  
  // If user has added items in all categories, mark as 100% complete
  const allCategoriesComplete = hasAssets && hasDocuments && hasWishes && hasExecutors;
  
  // Calculate percentage based on checklist or actual progress, whichever is higher
  ...
  const completionPercentage = allCategoriesComplete ? 100 : Math.max(checklistPercentage, actualProgressPercentage);
        ...
        <CardHeader>
          ...
          <CardDescription>
            {allCategoriesComplete 
              ? "You've completed all essential planning tasks!" 
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              <div className={`p-3 rounded-lg border ${hasAssets ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                <div className="flex items-center gap-2">
                  <FolderClosed className={`h-5 w-5 ${hasAssets ? 'text-green-500' : 'text-gray-400'}`} />
                ...
                <div className="mt-1 text-sm">
                  {hasAssets ? (
                    ...
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className="p-0 h-auto text-calm-600"
              ...
              <div className={`p-3 rounded-lg border ${hasDocuments ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                <div className="flex items-center gap-2">
                  <FileText className={`h-5 w-5 ${hasDocuments ? 'text-green-500' : 'text-gray-400'}`} />
              ...
              <div className={`p-3 rounded-lg border ${hasWishes ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                <div className="flex items-center gap-2">
                  <HeartHandshake className={`h-5 w-5 ${hasWishes ? 'text-green-500' : 'text-gray-400'}`} />
              ...
              <div className={`p-3 rounded-lg border ${hasExecutors ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                <div className="flex items-center gap-2">
                  <Users className={`h-5 w-5 ${hasExecutors ? 'text-green-500' : 'text-gray-400'}`} />
